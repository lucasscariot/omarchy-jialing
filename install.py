...
def write_files(files, state):
    ...
    for index, (path, data) in enumerate(files.items()):
        ...
        entries.append({'path':str(path), 'old':str(index) if old is not None else None,
...
def profile_files(config, state, variant):
    files = {}
    ...
    include = '<!-- Jialing begin -->\n<include ignore_missing="yes">' + escape(str(rules)) + '</include>\n<!-- Jialing end -->\n'
    ...
    for toolkit in ('gtk-3.0','gtk-4.0'):
        ...
        buffer = io.StringIO(); ini.write(buffer)
...
def daylight_files(config, state, data, auto_location):
    ...
    if not location.get('auto_location'):
    ...
    return {script: (ROOT/'daylight.py').read_bytes(),
...
def main():
    ...
    parser.add_argument('--no-apply', action='store_true', help='Write files only; no desktop commands')
    parser.add_argument('--daylight', action='store_true', help='Install optional scheduler files; enable timer separately')
    parser.add_argument('--auto-location', action=argparse.BooleanOptionalAction, default=None, help='Opt into hourly ipapi.co lookup (requires --daylight)')
    parser.add_argument('--profile', action='store_true', help='Opt into global font and Ghostty settings')
    ...
    files = {config/'omarchy/themes'/theme/name: (ROOT/'themes'/theme/name).read_bytes()
             for theme in THEMES for name in ASSETS}
    if args.wallpaper:
        ...
        files.update({config/'omarchy/themes'/theme/'backgrounds'/args.wallpaper.name:wallpaper for theme in THEMES})
    if args.profile:
        files.update(profile_files(config, state, args.variant))
    if args.daylight:
        files.update(daylight_files(config, state, location_root('XDG_DATA_HOME', '.local/share'), args.auto_location))
    backup = write_files(files, state)
    print(json.dumps({'backup':str(backup), 'themes':list(THEMES)}), flush=True)
    if not args.no_apply:
        subprocess.run(['omarchy','theme','set', 'jialing-light' if args.variant=='light' else 'jialing'], check=True, stdout=subprocess.DEVNULL)
