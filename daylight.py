...
def validate_location(value):
    ...
    return {key: value[key] for key in ("latitude", "longitude", "timezone")} | {
        "city": str(value.get("city") or "Configured location")
    }
...
def current_location(config, cache_path, refresh=False, force=False):
    ...
    if refresh and (force or not 0 <= now - checked < LOCATION_TTL):
        try:
            request = urllib.request.Request(
                "https://ipapi.co/json/", headers={"User-Agent": "JialingDaylight/1.0"}
            )
    ...
    try:
        ...
        return location | {
            "location_source": "cached ipapi.co",
            "location_warning": cached.get("error"),
        }
    except ValueError:
        return validate_location(config) | {
            "location_source": "configured fallback",
            "location_warning": cached.get("error"),
        }
...
def selection(location, now):
    ...
    return {
        "theme": "jialing-light" if day else "jialing",
        "sunrise": rise.isoformat() if rise else None,
        "sunset": setting.isoformat() if setting else None,
    }


def current_theme(state):
    try:
        return (state / "omarchy/current/theme.name").read_text().strip()
    except OSError:
        return None
...
def main():
    ...
    parser.add_argument(
        "--status",
        action="store_true",
        help="Inspect schedule; no desktop or network changes by default",
    )
    parser.add_argument(
        "--refresh-location",
        action="store_true",
        help="Request an IP location refresh when auto_location is enabled",
    )
    ...
    if subprocess.run(
        ...
        return
    # Location lookup may take seconds. Respect a manual change made meanwhile.
    if current_theme(state) != current:
        return
    # Omarchy serializes the actual update; an external change can still race this check.
    subprocess.run(
        ["omarchy", "theme", "set", result["theme"]],
