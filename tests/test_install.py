...
class InstallTests(unittest.TestCase):
    ...
    def test_symlink_destination_is_rejected_before_any_theme_is_written(self):
        ...
        outside.mkdir()
        target = self.config / "omarchy/themes/jialing-light"
    ...
    def test_optional_daylight_install_preserves_location_and_is_restorable(self):
        ...
        original = (
            '{"latitude": 48.85, "longitude": 2.35, "timezone": "Europe/Paris", "custom": true}'
        )
    ...
    def test_local_wallpaper_is_installed_for_both_variants_and_restored(self):
        ...
        for theme in ("jialing", "jialing-light"):
            self.assertEqual(
                (self.config / "omarchy/themes" / theme / "backgrounds/city.jpg").read_bytes(),
                b"local image",
            )
        self.run_cli("--restore", report["backup"])
        self.assertFalse((self.config / "omarchy/themes/jialing/backgrounds/city.jpg").exists())

    def test_incomplete_backup_refuses_restore_before_changing_any_files(self):
        original = self.config / "omarchy/themes/jialing-light/colors.toml"
        original.parent.mkdir(parents=True)
        original.write_text("old light palette")
        report = self.run_cli()
