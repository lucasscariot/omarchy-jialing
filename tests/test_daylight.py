...
class DaylightTests(unittest.TestCase):
    ...
    def status(self, now):
        result = subprocess.run(
            [sys.executable, str(ROOT / "daylight.py"), "--status", "--now", now],
            env=self.env,
            capture_output=True,
            text=True,
        )
    ...
    def test_invalid_coordinates_fail_without_desktop_changes(self):
        ...
        result = subprocess.run(
            [sys.executable, str(ROOT / "daylight.py"), "--status"],
            env=self.env,
            capture_output=True,
            text=True,
        )
    ...
    def test_failed_location_lookup_uses_configured_fallback(self):
        ...
        with (
            ...
            patch.object(sys, "argv", ["daylight.py", "--status", "--refresh-location"]),
    ...
    def test_scheduler_caches_successful_lookup_for_an_hour_without_storing_ip(self):
        ...
        with (
            patch.dict(os.environ, self.env),
            patch.object(sys, "argv", ["daylight.py"]),
            patch("urllib.request.urlopen", return_value=response) as lookup,
    ...
    def test_missing_location_fails_clearly_and_manual_override_skips_lookup(self):
        ...
        (self.home / "state/omarchy/current/theme.name").write_text("tokyo-night")
        result = subprocess.run(
            [sys.executable, str(ROOT / "daylight.py")],
            env=self.env,
            capture_output=True,
            text=True,
        )
    ...
    def test_manual_theme_selected_during_lookup_is_preserved(self):
        import contextlib
        import importlib.util
        import io
        from unittest.mock import MagicMock, patch
        ...
        # Try both initial variants so one must require a switch regardless of clock time.
        for initial in ("jialing", "jialing-light"):
            (self.home / "state/omarchy/current/theme.name").write_text(initial)
            (self.home / "state/omarchy/jialing-location.json").unlink(missing_ok=True)
            with (
                patch.dict(os.environ, self.env),
                patch.object(sys, "argv", ["daylight.py"]),
                patch("urllib.request.urlopen", side_effect=lookup),
                patch("subprocess.run", return_value=subprocess.CompletedProcess([], 0)) as command,
                contextlib.redirect_stdout(io.StringIO()),
            ):
                module.main()
                self.assertFalse(
                    any(call.args[0][0] == "omarchy" for call in command.call_args_list)
                )
